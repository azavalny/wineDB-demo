    ...
    const fetchUserCellar = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(
            `
            wine:wine_id(*, vineyard:vineyard_id(*))
          `
          )
  ...
  const handleRemove = async (wine: userWine) => {
    try {
      ...
      setUserWines((prevWines) =>
        prevWines.filter((w) => w.name !== wine.name)
      );
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#181818] text-[#f1f1f1] p-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold text-[#ffccbb] mb-4">
            My Wine Cellar
          </h1>
          <p className="text-xl text-[#ccc]">
            Here you can manage your wine collection
          </p>
        </div>
        ...
        {userWines.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-[#aaa]">
              Start adding wines from the home page to build your collection!
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {userWines.map((wine, index) => (
              <div
                key={index}
                className="bg-[#292929] rounded-lg p-6 border border-[#444] hover:bg-[#3a3a3a] transition-colors duration-200"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-2 gap-x-6 mb-4 text-[#ddd]">
                  <div>
                    <span className="font-semibold text-[#ffccbb]">
                      Classification:
                    </span>{" "}
                    {wine.classification}
                  </div>
                  <div>
                    <span className="font-semibold text-[#ffccbb]">Year:</span>{" "}
                    {wine.year}
                  </div>
                  <div>
                    <span className="font-semibold text-[#ffccbb]">
                ...
                {editingWine?.name === wine.name && (
                  <div className="bg-[#1f1f1f] rounded-lg p-4 mt-4 border border-[#555]">
                    <h3 className="font-bold mb-4 text-center text-[#f1f1f1]">
                      Edit Wine
                    </h3>
